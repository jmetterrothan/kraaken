...
import { linesIntersectionWithVector2 } from "./../utility/MathHelpers";
...
class Box2 {
  ...
  public intersectSegment(v1: Vector2, v2: Vector2): Vector2 | undefined {
    const p1 = linesIntersectionWithVector2(v1, v2, new Vector2(this.min.x, this.min.y), new Vector2(this.min.x, this.max.y));
    if (p1 !== undefined) {
      // console.log("intersected box left side");
      return p1;
    }
    const p2 = linesIntersectionWithVector2(v1, v2, new Vector2(this.max.x, this.min.y), new Vector2(this.max.x, this.max.y));
    if (p2 !== undefined) {
      // console.log("intersected box right side");
      return p2;
    }
    const p3 = linesIntersectionWithVector2(v1, v2, new Vector2(this.min.x, this.min.y), new Vector2(this.max.x, this.min.y));
    if (p3 !== undefined) {
      // console.log("intersected box top side");
      return p3;
    }
    const p4 = linesIntersectionWithVector2(v1, v2, new Vector2(this.min.x, this.max.y), new Vector2(this.max.x, this.max.y));
    if (p4 !== undefined) {
