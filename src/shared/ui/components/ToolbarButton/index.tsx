...
const ToolbarButton: React.FunctionComponent<IToolbarButtonprops> = ({ theme = "blue", name, icon, active, disabled, showCaret, onClick }) => {
  return (
    <button title={name} type="button" className={cx("toolbar-button", theme && `toolbar-button--${theme}`, disabled && "disabled", active && "active")} onClick={disabled ? undefined : onClick}>
